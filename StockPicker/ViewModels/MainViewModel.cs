            ...
            if (finnhubDs != null)
            {
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        // Refresh the grid on the UI thread so WPF bindings update.
                        // InvokeAsync instead of Invoke avoids blocking the thread-pool thread
                        // on the UI thread, eliminating a latent deadlock risk.
