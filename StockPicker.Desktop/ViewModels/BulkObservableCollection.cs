...
namespace StockPicker.Desktop.ViewModels
{
    ...
    /// </summary>
    /// <remarks>
    /// WPF-ADAPTATION: copied verbatim from <c>StockPicker/ViewModels/BulkObservableCollection.cs</c>;
    /// only the namespace changed (<c>StockPicker.ViewModels</c> → <c>StockPicker.Desktop.ViewModels</c>).
    /// The Reset-batching pattern is framework-agnostic (pure BCL
    /// <see cref="ObservableCollection{T}"/> + <see cref="INotifyCollectionChanged"/>). See the
    /// class summary in the phase report for the DataGridCollectionView Reset-observation note.
    /// </remarks>
