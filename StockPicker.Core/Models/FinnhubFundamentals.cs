namespace StockPicker.Models
{
    ...
    ///
    /// UNIT NOTE: Verified 2026-06-25 against a live free-tier key (AAPL/NVDA/KO/JNJ).
    /// series.annual values are: totalDebtToEquity = raw ratio (1.35 = 135%),
    /// netDebtToTotalEquity = raw ratio (negative = net cash), roe = fraction (0.41 = 41%).
    /// Raw values are still logged via Debug.WriteLine on first call, and display helpers
    /// in <see cref="StockPicker.Models.Recommendation"/> are kept in one place so a future
    /// unit change is a one-line edit.
    /// </summary>
    public class FinnhubFundamentals
    {
        /// <summary>
        /// Total debt to equity ratio from Finnhub series.annual.totalDebtToEquity.
        /// Verified unit: raw ratio (e.g. 1.5 = 150 % debt/equity).
        /// </summary>
        ...
        /// Verified unit: fraction (e.g. 0.15 = 15 % ROE).
